/**
 * .kryptix vault file format (AES-256-CBC + SHA-256 stretch KDF + MAC).
 * Pure TypeScript — shared by mobile and desktop.
 */
import type {
  KryptixSectionFlags,
  KryptixVaultFile,
...
import { sha256Hex, getRandomBytes, toHex, fromHex } from './webCrypto';
...
// ---- AES-256-CBC (same pure-JS tables as encryption.ts) ----
...
async function aesEncryptCbc(
  ...
  const key = Array.from(fromHex(keyHex));
  const w = keyExpansion(key);
  const ivArr = getRandomBytes(16);
...
async function aesDecryptCbc(
  ...
  const key = Array.from(fromHex(keyHex));
  const w = keyExpansion(key);
  const iv = Array.from(fromHex(ivHex));
  const data = Array.from(fromHex(ciphertextHex));
  const out: number[] = [];
  let prev = iv;
  for (let i = 0; i < data.length; i += 16) {
