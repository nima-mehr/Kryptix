/**
 * Pure encryption helpers shared by mobile + desktop.
 * AES-256-CBC is a pure-JS implementation (same as mobile).
 * SHA-256 + random bytes use Web Crypto API.
 */
import type { EncryptionAlgorithm } from '../types/hardcoded';
import { sha256Bytes, getRandomBytes, toHex, fromHex } from './webCrypto';
...
async function xorDecrypt(hex: string, passphrase: string): Promise<string> {
  const keyBytes = await sha256Bytes(passphrase);
  const data = Array.from(fromHex(hex));
  const out = data.map((b, i) => b ^ keyBytes[i % keyBytes.length]);
